...
#TODO Use fetchrow_hashref
	...
	foreach (@campaign_id) {
		...
		if ( $hopper_ready_count > 0 ) {
			...
			# Get stat averages
			$ready_diff_avg = $ready_diff_total / 60     if ( $ready_diff_total > 0);
			$waiting_diff_avg = $waiting_diff_total / 60 if ( $waiting_diff_total > 0);
			$total_agents_avg = $total_agents_total / 60 if ( $total_agents_total > 0);
