...
foreach(@campaign_id)
	...
	if ($hopper_ready_count < $hopper_level[$i])
		...
		if ( ($campaign_leads_to_call[$i] < 1) && ($rec_ct[$i] < 1) )
			...
			if ($lead_order[$i] =~ /^UP LAST NAME/) {$order_stmt = 'order by last_name desc, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^DOWN LAST NAME/) {$order_stmt = 'order by last_name, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^UP PHONE/) {$order_stmt = 'order by phone_number desc, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^DOWN PHONE/) {$order_stmt = 'order by phone_number, called_count asc, lead_id asc';}
			...
			if ($lead_order[$i] =~ /^DOWN TZ/) {$order_stmt = 'order by gmt_offset_now desc, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^UP TZ/) {$order_stmt = 'order by gmt_offset_now desc, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^UP TZ LAST NAME/) {$order_stmt = 'order by gmt_offset_now desc, last_name desc, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^DOWN TZ LAST NAME/) {$order_stmt = 'order by gmt_offset_now desc, last_name, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^UP TZ PHONE/) {$order_stmt = 'order by gmt_offset_now desc, phone_number desc, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^DOWN TZ PHONE/) {$order_stmt = 'order by gmt_offset_now desc, phone_number, called_count asc, lead_id asc';}
			if ($lead_order[$i] =~ /^UP TZ COUNT/) {$order_stmt = 'order by gmt_offset_now desc, called_count desc, lead_id asc';}
			if ($lead_order[$i] =~ /^DOWN TZ COUNT/) {$order_stmt = 'order by gmt_offset_now desc, called_count, lead_id asc';}
