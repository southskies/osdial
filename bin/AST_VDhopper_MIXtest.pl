...
foreach(@campaign_id)
	...
	if ($hopper_ready_count < $hopper_level[$i])
		...
		if ($list_order_mix[$i] =~ /DISABLED/)
			...
			{
			$stmtA = "SELECT count(*) FROM osdial_list where called_since_last_reset='N' and ($list_mix_dialableSQL) and ($all_gmtSQL[$i]) $lead_filter_sql[$i];";
			}
			if ($DBX) {print "     |$stmtA|\n";}
		$sthA = $dbhA->prepare($stmtA) or die "preparing: ",$dbhA->errstr;
		$sthA->execute or die "executing: $stmtA ", $dbhA->errstr;
		...
		### BEGIN standard grab leads ###
			...
			##### LIST MIX LEADS GRAB #####
					...
					foreach(@list_mixARY)
						...
						$stmtA = "SELECT lead_id,list_id,gmt_offset_now,phone_number,state FROM osdial_list where called_since_last_reset='N' and $list_mix_dialableSQL and lead_id NOT IN($lead_id_lists) and ($all_gmtSQL[$i]) $lead_filter_sql[$i] $order_stmt limit $LM_step_goal[$x];";
						if ($DBX) {print "     |$stmtA|\n";}
						$sthA = $dbhA->prepare($stmtA) or die "preparing: ",$dbhA->errstr;
						$sthA->execute or die "executing: $stmtA ", $dbhA->errstr;
						$sthArows=$sthA->rows;
