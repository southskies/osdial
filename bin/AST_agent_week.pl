...
foreach(@PAST_date)
{
	...
	while ($sthArows > $rec_count)
		...
		$stmtB = "SELECT SQL_NO_CACHE event_time,UNIX_TIMESTAMP(event_time) FROM $osdial_agent_log WHERE event_time<='$PAST_date[$h] 23:59:59' AND event_time>='$PAST_date[$h] 00:00:00' AND user='$aryA[2]' ORDER BY event_time LIMIT 1;";
		...
		$stmtB = "SELECT SQL_NO_CACHE event_time,UNIX_TIMESTAMP(event_time) FROM $osdial_agent_log WHERE event_time<='$PAST_date[$h] 23:59:59' AND event_time>='$PAST_date[$h] 00:00:00' AND user='$aryA[2]' ORDER BY event_time DESC LIMIT 1;";
		$sthB = $dbhB->prepare($stmtB) or die "preparing: ",$dbhB->errstr;
		$sthB->execute or die "executing: $stmtA ", $dbhB->errstr;
		@aryB = $sthB->fetchrow_array;
