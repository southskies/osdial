...
def main(argv):
    ...
    vmupdate_process(logger)


def vmupdate_process(logger):
    """
    ...
    try:
        ami = pystrix.ami.Manager(aggregate_timeout=10)
    ...
    for phone in phones:
        ...
        action = pystrix.ami.core.MailboxCount(mailbox=phone['voicemail_id'])
        action_id = 'P'+str(phone['voicemail_id'])+'~U'+str(time.time())+"~MailboxCount"
        res = ami.send_action(action, action_id=action_id, ActionID=action_id)
        resdata = res.result
        origmsgcnt = phone['messages']
        origomsgcnt = phone['old_messages']
        if resdata.has_key('NewMessages'):
            phone['messages'] = int(resdata['NewMessages'])
        if resdata.has_key('UrgMessages'):
            phone['messages'] += int(resdata['UrgMessages'])
        if resdata.has_key('OldMessages'):
            phone['old_messages'] = int(resdata['OldMessages'])
