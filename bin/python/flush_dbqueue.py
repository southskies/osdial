...
def main(argv):
    ...
    flushdbqueue_process(logger)


def flushdbqueue_process(logger):
    """
    The routine responsible for flushing the DB queues.
    """
    osdial = OSDial()
    six_hour = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time() - 21600))
    three_hour = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time() - 10800))
    quarter_hour = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time() - 900))
    eigth_hour = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time() - 450))
