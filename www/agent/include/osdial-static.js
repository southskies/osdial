					...
					if ( (MDlogLINE == "LOG NOT ENTERED") && (VDstop_rec_after_each_call != 1) ) {
						...
						if ( (taskMDstage != "start") && (VDstop_rec_after_each_call == 1) ) {
							...
							if (window.MDlogRecorDings) {
								...
								var RecDispNamE = '';
								RecDispNamE += MDlogRecorDings_array[2];
...
// Request list of USERONLY callbacks for this agent
	function CalLBacKsLisTCheck() {
		...
		if ( (VD_live_customer_call==1) || (alt_dial_active==1) ) {
			...
			document.getElementById('CallBacKsLisTBox').style.zIndex="38";
...
// Open up a callback customer record as manual dial preview mode
	function new_callback_call(taskCBid,taskLEADid) {
		...
		document.getElementById('CallBacKsLisTBox').style.zIndex="0";
...
// Fast version of manual dial
	function NeWManuaLDiaLCalLSubmiTfast() {
		...
		if ( (manual_dialcode.length < 1) || (MDPhonENumbeRform.length < 5) ) {
			...
			document.osdial_form.DiaLAltPhonE.checked=true;
			ManualDialNext("","",manual_dialcode,MDPhonENumbeRform,MDLookuPLeaD,'fast');
		}
	}
...
// Request lookup of manual dial channel
	function ManualDialCheckChanneL(taskCheckOR) {
		...
		var xmlhttp=getXHR();
		if (xmlhttp) { 
			manDiaLlook_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=manDiaLlookCaLL&conf_exten=" + session_id + "&user=" + user + "&pass=" + pass + "&MDnextCID=" + CIDcheck + "&agent_log_id=" + agent_log_id + "&lead_id=" + document.osdial_form.lead_id.value + "&DiaL_SecondS=" + MD_ring_secondS + "&stage=" + taskCheckOR + "&campaign=" + campaign;
				...
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					if (MDlookCID == "NO") {
						...
						if (taskCheckOR == 'YES') {
							...
							XDerror = MDlookResponse_array[2];
							XDstatus = MDlookResponse_array[3];
							XDhangupcause = MDlookResponse_array[4];
							if (XDerror == 'ERROR') {
								document.osdial_form.xferuniqueid.value	= XDuniqueid;
								document.osdial_form.xferchannel.value	= XDchannel;
								lastxferchannel = XDchannel;
								document.osdial_form.xferlength.value		= 0;
								XD_live_call_secondS = 0;
								osdalert("Call Rejected: "+XDhangupcause+"  Status: "+XDstatus+"\n",5);
								xfercall_send_hangup();
								document.getElementById("MainStatuSSpan").innerHTML = "<font color=" + status_alert_color + "> Call Rejected: " + XDhangupcause + "&nbsp;&nbsp;Status: "+XDstatus+"</font>";
							} else if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && ((MD_ring_secondS < 10 && document.osdial_form.xfernumber.value!='CXFER') || (MD_ring_secondS < 1 && document.osdial_form.xfernumber.value=='CXFER'))) {
								...
								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><span class=\"XferLeave3wayButton\">Leave 3WAY Call</span></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<span class=\"XferDialButtonOff\">Dial With Customer</span>";

								document.getElementById("ParkCustomerDial").innerHTML ="<span class=\"XferParkDialButtonOff\">Park Customer Dial</span>";

								document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><span class=\"XferHangupButton\">Hangup XFER Line</span></a>";

								document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"bothcall_send_hangup();return false;\"><span class=\"XferHangupBothButton\">Hangup Both Lines</span></a>";

								document.getElementById("DialBlindTransfer").innerHTML ="<span class=\"XferBlindDialButtonOff\">Dial Blind Transfer</span>";
								document.getElementById("LocalCloser").innerHTML ="<span class=\"XferLocalCloserButtonOff\">Local Closer</span>";
						...
						// Manual dials (MD)
							...
							if (MDerror == 'ERROR') {
								...
								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><span class=\"XferLocalCloserButton\">Local Closer</span></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><span class=\"XferBlindDialButton\">Dial Blind Transfer</span></a>";
...
// Set the client to READY and start looking for calls (VDADready, VDADpause)
	function AutoDial_ReSume_PauSe(taskaction,taskagentlog,taskwrapup) {
		...
		if (agent_log_time==0 && agent_log_time_total>0 && agent_log_type=="PAUSE") agent_log_type="DISPO";
		if (agent_log_time>0) {
			logTimeTrans();
		}
	...
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo,nodeletevdac) {
		...
		if (form_cust_channel.length > 3) {
			...
			if (xmlhttp) { 
				...
				debug("<b>custhangup_query:</b>" + custhangup_query,3);
				xmlhttp.open('POST', 'manager_send.php'); 
				xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() {
		...
		if (process_post_hangup==1) {
			...
			document.getElementById("Leave3WayCall").innerHTML ="<span class=\"XferLeave3wayButtonOff\">Leave 3WAY Call</span>";

			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><span class=\"XferDialButton\">Dial With Customer</span></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"xfer_park_dial();return false;\"><span class=\"XferParkDialButton\">Park Customer Dial</span></a>";

			document.getElementById("HangupXferLine").innerHTML ="<span class=\"XferHangupButton\">Hangup XFER Line</span>";
	...
	function DialTimeHangup(tasktypecall) {
		...
		if (RedirecTxFEr < 1 && leaving_threeway<1) {
			...
			if (xmlhttp) { 
				...
				custhangup_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=HangupConfDial&format=text&user=" + user + "&pass=" + pass + "&exten=" + session_id + "&ext_context=" + ext_context + "&queryCID=" + queryCID + "&CalLCID=" + CalLCID + "&call_server_ip=" + lastcustserverip;
				debug("<b>custhangup_query:</b>" + custhangup_query,3);
				xmlhttp.open('POST', 'manager_send.php'); 
				xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
				xmlhttp.send(custhangup_query); 
...
// Generate the Pause Code Chooser panel
	function PauseCodeSelectContent_create() {
		...
		if ( (VD_live_customer_call==1) || (alt_dial_active==1) ) {
			...
			PauseCode_HTML = PauseCode_HTML + "</span></font></td></tr></table><BR><BR><font size=3 \"><b><a href=\"#\" onclick=\"if (PCSpause==1) {AutoDial_ReSume_PauSe('VDADready','NEW_ID');} PauseCodeSelect_submit('');return false;\">Go Back</a>";
...
// Log the user out of the system, if active call or active dial is occuring, don't let them.
	function LogouT(tempreason) {
		...
		if (1==2) {
			document.getElementById("LogouTBoxLink").innerHTML = "<span class=homepage><div class=loginagainBox><span class=LoginAgainHeader>Log Back In?</span><br/><br/><a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + encodeURIComponent2(session_id) + "&session_name=" + encodeURIComponent2(session_name) + "&VD_login=" + encodeURIComponent2(user) + "&VD_campaign=" + encodeURIComponent2(campaign) + "&phone_login=" + encodeURIComponent2(phone_login) + "&phone_pass=" + encodeURIComponent2(phone_pass) + "&VD_pass=" + encodeURIComponent2(pass) + "\">Yes</a>&nbsp;/&nbsp;<a href=\""+agcPAGE+"?logout=1\">No</a></div></span>";
		} else {
...
// GLOBAL FUNCTIONS
	...
	function start_all_refresh() {
		...
		if (OSDiaL_closer_login_checked==0) {
			...
			agent_log_time=0;
			agent_log_epoch=agent_log_epoch_start=Math.floor(new Date().getTime()/1000);
		...
		start_all_timeout_id = setTimeout("all_refresh()", refresh_interval);
	}
	...
	function getTimeTransStats() {
		if (company_id>0) {
			var xmlhttp=getXHR();
			gettime_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=getTimeTransStats&conf_exten=" + session_id + "&user=" + user + "&pass=" + pass + "&campaign=" + campaign + "&agent_log_id=" + agent_log_id + "&agent_log_type=" + agent_log_type + "&agent_log_time=" + agent_log_time + "&company_id=" + company_id;
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
			xmlhttp.send(gettime_query); 
			xmlhttp.onreadystatechange = function() { 
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					if (company_status!='ACTIVE') {
						...
						if (company_status=='SUSPENDED') {
							osdalert("Logged-out user!<br>Company profile is no longer active!!!<br>The company account has been suspended for insufficient funds. Please tell your manager or system administrator.", 180);
						} else if (company_status=='TERMINATED') {
							osdalert("Logged-out user!<br>Company profile is no longer active!!!<br>The company profile has been removed. Please tell your manager or system administrator.", 180);
						} else {
	...
	function timeTransCounter() {
		agentlogtime_timeout_id = setTimeout("timeTransCounter()", 1000);
		var ttcepoch=Math.round(new Date().getTime()/1000);
		if (agent_log_epoch_start==0) agent_log_epoch_start=ttcepoch;
		agent_log_epoch=ttcepoch;
		agent_log_time_total=(agent_log_epoch-agent_log_epoch_start);
		if (agent_log_time_total==0 || agent_log_time_total%60!=0) {
			agent_log_time=(agent_log_time_total%60);
		} else {
			agent_log_time=60;
			logTimeTrans();
			if (agent_log_type=='PAUSE' || agent_log_type=='WAIT') {
				getTimeTransStats();
			}
			agent_log_time=0;
		}
...
// Send the Manual Dial Next Number request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdnFlag) {
		debug("<b>ManualDialNext:</b> mdnCBid=" + mdnCBid + " mdnBDleadid=" + mdnBDleadid + " mdnDiaLCodE=" + mdnDiaLCodE + " mdnPhonENumbeR=" + mdnPhonENumbeR + " mdnStagE=" + mdnStagE + " mdnFlag=" + mdnFlag,2);
		dial_timedout=0;
		if (previewFD_time > 0) {
			clearTimeout(previewFD_timeout_id);
			clearInterval(previewFD_display_id);
		...
		if (xmlhttp) { 
			...
			manDiaLnext_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=manDiaLnextCaLL&conf_exten=" + session_id + "&user=" + user + "&pass=" + pass + "&campaign=" + campaign + "&ext_context=" + ext_context + "&dial_context=" + dial_context + "&dial_timeout=" + dial_timeout + "&dial_prefix=" + dial_prefix + "&campaign_cid=" + cid + "&campaign_cid_name=" + cid_name + "&preview=" + man_preview + "&agent_log_id=" + agent_log_id + "&callback_id=" + mdnCBid + "&lead_id=" + mdnBDleadid + "&phone_code=" + mdnDiaLCodE + "&phone_number=" + mdnPhonENumbeR + "&list_id=" + mdnLisT_id + "&stage=" + mdnStagE  + "&use_internal_dnc=" + use_internal_dnc + "&omit_phone_code=" + omit_phone_code + "&inbound_man=" + inbound_man + "&mdnFlag=" + mdnFlag + "&use_cid_areacode_map=" + use_cid_areacode_map;
			...
			xmlhttp.onreadystatechange = function() { 
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					if ( (MDnextCID.match(regMNCvar)) || (MDnextCID.match(regMDFvarDNC)) || (MDnextCID.match(regMDFvarCAMP)) ) {
						...
						if (mdnFlag=='fast' || man_preview=='NO') {
							if (agent_log_type=="PAUSE") {
								logTimeTrans();
								agent_log_type="WAIT";
								agent_log_time=0;
	...
	function DispoSelect_submit() {
		...
		if (DispoChoice.length < 1) {
			...
			if ( (DispoChoice == 'CALLBK') && (scheduled_callbacks > 0) ) {
				...
				var xmlhttp=getXHR();
				if (xmlhttp) { 
					DSupdate_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=updateDISPO&format=text&user=" + user + "&pass=" + pass + "&dispo_choice=" + DispoChoice + "&lead_id=" + document.osdial_form.lead_id.value + "&campaign=" + campaign + "&auto_dial_level=" + auto_dial_level + "&agent_log_id=" + agent_log_id + "&PostDatETimE=" + PostDatETimE + "&CallBackDatETimE=" + CallBackDatETimE + "&list_id=" + document.osdial_form.list_id.value + "&recipient=" + CallBackrecipient + "&use_internal_dnc=" + use_internal_dnc + "&MDnextCID=" + LasTCID + "&stage=" + group + "&phone_gmt=" + phone_gmt + "&phone_local_gmt=" + local_gmt + "&comments=" + CallBackCommenTs;
		...
		if (scriptformat == 'YES') {
			...
			for (var i=0; i<AFids.length; i++) {
				...
				if (AFoptions[i] && AFoptions[i] != '') {
					...
					var options = AFoptions[i].split(',');
					for (var option in options) {
						SCt += "<option>" + options[option] + "</option>";
					}
					SCt += "</select>";
				} else if (AFlengths[i] && AFlengths[i]=='0') {
					SCt += "<textarea style=\"height:32px;font-size:11px;\" cols=\"22\" rows=\"2\" name="+AFnames[i];
					SCt += " onfocus=\"this.value=document.getElementById('" + AFids[i] + "').value;\"";
					SCt += " onchange=\"var afv=this; document.getElementById('" + AFids[i] + "').value=afv.value;";
					SCt += " var aflist=document.getElementsByName('" + AFnames[i] + "');";
					SCt += " for(var afli=0;afli<aflist.length;afli&#43;&#43;)";
					...
					SCt += "<input type=text size="+AFlengths[i]+" maxlength=255 name="+AFnames[i];
					SCt += " onfocus=\"this.value=document.getElementById('" + AFids[i] + "').value;\"";
					SCt += " onchange=\"var afv=this; document.getElementById('" + AFids[i] + "').value=afv.value;";
					SCt += " var aflist=document.getElementsByName('" + AFnames[i] + "');";
					SCt += " for(var afli=0;afli<aflist.length;afli&#43;&#43;)";
...
// formatPhone() - Format the given number for locale (phone_code).
	function formatPhone(phcode, phnum) {
		...
		// Sweden
			...
			var tac1 = phnum.substring(0,1);
			var tac = phnum.substring(0,2);
			if (tac1=='0') {
			...
			if (xmlhttp) { 
				...
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					if (MCsplit[0]!='') {
						...
						multicall_waitagentlogid = MCsplit[18];
						multicall_waitagentlogstate = MCsplit[19];
						multicall_waitagentlogtime = MCsplit[20];
						if (VD_live_customer_call==1 && multicall_lastchannel=='') {
							...
							multicall_lastagentlogid = agent_log_id;
							multicall_lastagentlogtype = agent_log_type;
							multicall_lastagentlogtime = agent_log_time;
						}
						var catab = '<table>';
	...
	function multicall_queue_swap() {
		...
		if (multicall_waitchannel!='') {
			if (multicall_channel=='' && multicall_lastchannel=='') {
				...
				multicall_agentlogid = multicall_waitagentlogid;
				multicall_agentlogstate = multicall_waitagentlogstate;
				multicall_agentlogtime = multicall_waitagentlogtime;
				...
				multicall_lastagentlogid = multicall_agentlogid;
				multicall_lastagentlogstate = multicall_agentlogstate;
				multicall_lastagentlogtime = multicall_agentlogtime;
			...
			multicall_waitchannel = multicall_waitserverip = multicall_waitcallerid = multicall_waituniqueid = "";
			multicall_waitleadid = multicall_waitingroup = multicall_waitvoicemail = multicall_waitagentlogid = multicall_waitagentlogstate = "";
			multicall_waitagentlogtime = 0;
			multicall_vmdrop_timer = -1;
		}
		...
		if (xmlhttpqs) { 
			...
			checkMCIN_query = "server_ip=" + lastcustserverip + "&channel=" + lastcustchannel + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass + "&campaign=" + campaign + "&ACTION=multicallQueueSwap" + "&agent_log_id=" + agent_log_id + "&agent_log_type=" + agent_log_type + "&agent_log_time=" + agent_log_time + "&agentchannel=" + agentchannel + "&agentserver_ip=" + server_ip + "&multicall_channel=" + multicall_channel + "&multicall_serverip=" + multicall_serverip + "&multicall_leadid=" + multicall_leadid + "&multicall_callerid=" + multicall_callerid + "&multicall_uniqueid=" + multicall_uniqueid + "&multicall_agentlogid=" + multicall_agentlogid + "&multicall_agentlogstate=" + multicall_agentlogstate + "&multicall_agentlogtime=" + multicall_agentlogtime + "&multicall_liveseconds=" + multicall_liveseconds + "&conf_exten=" + session_id + "&user_abb=" + user_abb + "&park_on_extension=" + park_on_extension;
			...
			if (xmlhttpqs.readyState == 4 && xmlhttpqs.status == 200) {
				...
				if (check_MCIC_array[0] == '1' || check_MCIC_array[0] == '2') {
					...
					multicall_tmpagentlogid = multicall_agentlogid;
					multicall_tmpagentlogstate = multicall_agentlogstate;
					multicall_tmpagentlogtime = multicall_agentlogtime;
					...
					multicall_agentlogid = multicall_lastagentlogid;
					multicall_agentlogstate = multicall_lastagentlogstate;
					multicall_agentlogtime = multicall_lastagentlogtime;
					...
					multicall_lastagentlogid = multicall_tmpagentlogid;
					multicall_lastagentlogstate = multicall_tmpagentlogstate;
					multicall_lastagentlogtime = multicall_tmpagentlogtime;
					...
					logTimeTrans();
					agent_log_id = VDIC_data_MCAC[6];
					agent_log_type = VDIC_data_MCAC[7];
					agent_log_time = (VDIC_data_MCAC[8]*1);
					agent_log_epoch_start=agent_log_epoch;
					logTimeTrans();
...
// Request hopper list for this campaign
	function MDHopperListCheck() {
		...
		if ( (VD_live_customer_call==1) || (alt_dial_active==1) ) {
			...
			document.getElementById('MDHopperListBox').style.zIndex='39';
...
// Open up a hopper record as manual dial preview mode
	function new_mdhopper_call(taskLEADid) {
		...
		document.getElementById('MDHopperListBox').style.zIndex='0';
