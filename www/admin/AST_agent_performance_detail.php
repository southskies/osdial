...
if( (strlen($PHP_AUTH_USER)<2) or (strlen($PHP_AUTH_PW)<2) or (!$auth)) {
    ...
    $html .= "Invalid Username/Password: |$PHP_AUTH_USER|$PHP_AUTH_PW|\n";
    echo $html;
    exit;
}
...
$html .= "  <title>OSDIAL: Agent Performance</title>\n";
$html .= "</head>\n";
$html .= "<body>\n";
...
if (!$group) {
    ...
    $plain .= "+-----------------+----------+--------+--------+---------+--------+--------+--------+--------+--------+--------+--------+--------+$statusesHEAD\n";
    $plain .= "|    USER NAME    |    ID    | CALLS  |NEWCALLS|  TIME   | PAUSE  |PAUSEAVG|  WAIT  | WAITAVG|  TALK  | TALKAVG| DISPO  | DISPAVG|$statusesHTML\n";
    $plain .= "+-----------------+----------+--------+--------+---------+--------+--------+--------+--------+--------+--------+--------+--------+$statusesHEAD\n";
    ...
    while ($m < $k) {
        ...
        $plain .= "| $Sfull_name | $Suser | $Scalls | $Snew_calls | $pfUSERtime_MS | $pfUSERtotPAUSE_MS | $pfUSERavgPAUSE_MS | $pfUSERtotWAIT_MS | $pfUSERavgWAIT_MS | $pfUSERtotTALK_MS | $pfUSERavgTALK_MS | $pfUSERtotDISPO_MS | $pfUSERavgDISPO_MS |$SstatusesHTML\n";
        ...
        if (count($SstatusesARY) > 0) {
            ...
            foreach ($SstatusesARY as $sa1) {
                $eline .= '|' . $sa1;
                $table .= "          <td align=right style=\"font-family: monospace;\">$sa1</td>\n";
            }
        }
        $table .= "        </tr>\n";
    ...
    $plain .= "+-----------------+----------+--------+--------+---------+--------+--------+--------+--------+--------+--------+--------+--------+$statusesHEAD\n";
    $plain .= "|  TOTALS        AGENTS:$TOT_AGENTS | $TOTcalls| $TOTnew_calls | $TOTtime_MS|$TOTtotPAUSE_MS| $TOTavgPAUSE_MS |$TOTtotWAIT_MS| $TOTavgWAIT_MS |$TOTtotTALK_MS| $TOTavgTALK_MS |$TOTtotDISPO_MS| $TOTavgDISPO_MS |$SUMstatusesHTML\n";
    $plain .= "+----------------------------+--------+--------+---------+--------+--------+--------+--------+--------+--------+--------+--------+$statusesHEAD\n";
    $plain .= "\n";
    ...
    if (count($SUMstatusesARY) > 0) {
        ...
        foreach ($SUMstatusesARY as $ss1) {
            $table .= "          <td align=right style=\"font-family: monospace;\"n owrap>$ss1</td>\n";
        }
    }
    $table .= "        </tr>\n";
    $table .= "      </table>\n";
