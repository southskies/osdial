...
function report_realtime_detail() {
    ...
    if ($group) {
        ...
        if ($LOG['multicomp']>0) {
            $tcnt=0;
            $tgroup = (OSDsubstr($group,0,3)*1)-100;
            if (OSDpreg_match("/^XXXX/",$group)) $tgroup=$LOG['company']['id'];
            $comp = get_first_record($link, 'osdial_companies', "*,IF(acct_enddate!='0000-00-00 00:00:00',DATEDIFF(acct_enddate,NOW()),0) AS daystillend", sprintf("id='%s'",mres($tgroup)) );
    ...
    */
    ...
    if ($talking_to_print > 0) {
        ...
        while ($j < $talking_to_print) {
            ...
            $userlink = "$user";
            if ($LOG['view_agent_status']) $userlink = "<a href=\"#\" onclick=\"if (ctrl_key) { openNewWindow('$PHP_SELF?ADD=999999&SUB=22&agent=$Luser&close_after_emergency_logout=Y&stage=log_agent_out&server_ip=$server_ip&conf_exten=$sessionid&extension=$extension&agent=$user'); } else { openNewWindow('$PHP_SELF?ADD=999999&SUB=22&agent=$Luser&close_after_emergency_logout=Y'); } return false;\" target=\"_blank\">$G$user$EG</a>";

            if ($disp_agent) $Ahtml .= "$LNleft<span name=agent_bar>$G <span name=agent_extension style=\"width:78px;display:inline-block;text-align:left;\">$extension</span> $LNcenterbar <span name=agent_user style=\"width:108px;display:inline-block;text-align:left;\"><a href=\"#\" onclick=\"if (ctrl_key) { openNewWindow('$PHP_SELF?ADD=999999&SUB=22&agent=$Luser&close_after_emergency_logout=Y&stage=log_agent_out&server_ip=$server_ip&conf_exten=$sessionid&extension=$extension&agent=$user'); } else { openNewWindow('$PHP_SELF?ADD=999999&SUB=22&agent=$Luser&close_after_emergency_logout=Y'); } return false;\" target=\"_blank\">$G$user$EG</a></span> $LNcenterbar $UGD<span name=agent_session style=\"width:54px;display:inline-block;text-align:left;\">$sessionid</span>$L$R $LNcenterbar <span name=agent_status style=\"width:54px;display:inline-block;text-align:left;\">$status $CM</span>$LNcenterbar $pausecode$SVD<span name=agent_time style=\"width:42;display:inline-block;text-align:left;\">".gmdate('G:i:s', $call_time_S)."</span> $LNcenterbar <span name=agent_campaign style=\"width:78px;display:inline-block;text-align:left;\">$campaign_id</span> $LNcenterbar <span name=agent_calls style=\"width:30px;display:inline-block;text-align:right;\">$calls_today</span> $LNcenterbar <span name=agent_ingroup style=\"width:120px;display:inline-block;text-align:right;\">$INGRP</span>$EG</span>$LNright\n";
