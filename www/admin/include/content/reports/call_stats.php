...
function report_call_stats() {
    ...
    if ($use_closer_log) {
        ...
        $ccprint2 = OSDpreg_replace("/,/",", ",$ccprint);
        $ccprint2 = OSDpreg_replace("/'/","    ",$ccprint2);
        $ccprint = OSDpreg_replace("/,/","\n",$ccprint);
        $ccprint = OSDpreg_replace("/'/","    ",$ccprint);
    ...
    while(OSDstrlen($TOTCATconversion)>7) {$TOTCATconversion = OSDsubstr("$TOTCATconversion", 0, -1);}
    ...
    if ($comment_grouping) {
        ...
        foreach ($cnvrate as $edkey => $edval) {
            ...
            if ($CATcontact > 0) {
                ...
                while(OSDstrlen($CATconversion)>7) {$CATconversion = OSDsubstr("$CATconversion", 0, -1);}
                $CATcontact =    sprintf("%5s", $CATcontact);
                while(OSDstrlen($CATcontact)>5) {$CATcontact = OSDsubstr("$CATcontact", 0, -1);}
                $CATdnc =    sprintf("%5s", $CATdnc);
                while(OSDstrlen($CATdnc)>5) {$CATdnc = OSDsubstr("$CATdnc", 0, -1);}
                $CATsale =    sprintf("%5s", $CATsale);
                while(OSDstrlen($CATsale)>5) {$CATsale = OSDsubstr("$CATsale", 0, -1);}
                $ed =    sprintf("%-50s", $edkey); 
                while(OSDstrlen($ed)>50) {$ed = OSDsubstr("$ed", 0, -1);}    
        ...
        if ($TOTCATcontact > 0) {
            ...
            while(OSDstrlen($TOTCATcontact)>5) {$TOTCATcontact = OSDsubstr("$TOTCATcontact", 0, -1);}
            $TOTCATdnc =    sprintf("%5s", $TOTCATdnc);
            while(OSDstrlen($TOTCATdnc)>5) {$TOTCATdnc = OSDsubstr("$TOTCATdnc", 0, -1);}
            $TOTCATsale =    sprintf("%5s", $TOTCATsale);
            while(OSDstrlen($TOTCATsale)>5) {$TOTCATsale = OSDsubstr("$TOTCATsale", 0, -1);}
            $plain .= "+----------------------------------------------------+----------+----------+----------+----------+\n";
            $plain .= "+ TOTALS                                             ";
    ...
    if ($use_closer_log) {
        $stmt=sprintf("SELECT Tuser,full_name,count(*),sum(Slength_in_sec),avg(Alength_in_sec) FROM ((SELECT osdial_log.user AS Tuser,full_name,uniqueid,length_in_sec AS Slength_in_sec,length_in_sec AS Alength_in_sec FROM osdial_log JOIN osdial_users ON (osdial_log.user=osdial_users.user) WHERE call_date BETWEEN '%s' AND '%s' %s AND osdial_log.user IS NOT NULL AND length_in_sec IS NOT NULL AND osdial_log.status IS NOT NULL) UNION (SELECT osdial_closer_log.user AS Tuser,full_name,uniqueid,length_in_sec AS Slength_in_sec,length_in_sec AS Alength_in_sec FROM osdial_closer_log JOIN osdial_users ON (osdial_closer_log.user=osdial_users.user) WHERE call_date BETWEEN '%s' AND '%s' %s AND osdial_closer_log.user IS NOT NULL AND length_in_sec IS NOT NULL AND osdial_closer_log.status IS NOT NULL)) AS t GROUP BY Tuser;",mres($query_date_BEGIN),mres($query_date_END),$group_SQLand,mres($query_date_BEGIN),mres($query_date_END),$closer_SQLand);
    } else if ($use_agent_log) {
        $stmt=sprintf("SELECT osdial_agent_log.user,full_name,count(*),sum(talk_sec),avg(talk_sec) FROM osdial_agent_log JOIN osdial_users ON (osdial_agent_log.user=osdial_users.user) WHERE event_time BETWEEN '%s' AND '%s' %s AND osdial_agent_log.user IS NOT NULL AND talk_sec IS NOT NULL AND osdial_agent_log.status IS NOT NULL GROUP BY osdial_agent_log.user;",mres($query_date_BEGIN),mres($query_date_END),$group_SQLand);
    } else {
        $stmt=sprintf("SELECT osdial_log.user,full_name,count(*),sum(length_in_sec),avg(length_in_sec) FROM osdial_log JOIN osdial_users ON (osdial_log.user=osdial_users.user) WHERE call_date BETWEEN '%s' AND '%s' %s AND osdial_log.user IS NOT NULL AND length_in_sec IS NOT NULL AND osdial_log.status IS NOT NULL GROUP BY osdial_log.user;",mres($query_date_BEGIN),mres($query_date_END),$group_SQLand);
    }
    $rslt=mysql_query($stmt, $link);
    if ($DB) {$html .= "$stmt\n";}
