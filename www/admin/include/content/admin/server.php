            ...
            if ($row[0] > 0 && $server_ip != $old_server_ip) {
                ...
                if (OSDstrlen($server_id) < 1 or OSDstrlen($server_ip) < 7) {
                    ...
                    $stmt = sprintf("UPDATE servers SET server_id='%s',server_description='%s',server_ip='%s',active='%s',asterisk_version='%s',max_osdial_trunks='%s',telnet_host='%s',".
                        ...
                        "balance_trunks_offlimits='%s',server_profile='%s',server_domainname='%s',server_public_ip='%s',web_url='%s',asterisk_languages='%s' WHERE server_id='%s';",
                        ...
                        mres($balance_trunks_offlimits),mres($server_profile),mres($server_domainname),mres($server_public_ip),mres($web_url),mres($asterisk_languages),mres($old_server_id));
