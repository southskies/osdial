...
OSDCP.ColorPicker = function () {
			...
			for (var i=0;i<inputs.length;i++) {
				if (inputs[i].getAttribute('mode')=='colorpicker') {
					...
					if (inputs[i].count === undefined) {
						...
						if (this.Input[count].getAttribute('forceWidth') > 0)
							inputwidth=this.Input[count].getAttribute('forceWidth');
						if (this.Input[count].getAttribute('hideInput') == '1')
							this.Input[count].type = 'hidden';
						...
						arrow.onmousedown=function() {this.src=arrowImageDown};
						arrow.onmouseover=function() {this.src=arrowImageOver};
						arrow.onmouseout=function() {this.src=arrowImage};
						arrow.onmouseup=function() {this.src=arrowImage};
	...
	onEvent(window,"load", function() {if(!picker) picker=new Picker;});
	return {
		refresh: function () {
