...
if [ "$?" -gt 0 ]; then echo -c "x"; else echo -n "."
...
echo "ALTER TABLE $db CHANGE COLUMN VICIDIAL_park_on_extension OSDIAL_park_on_extension VARCHAR(10) default '8301';" | mysql -u root > /dev/null 2>&1
if [ "$?" -gt 0 ]; then echo -c "x"; else echo -n "."
echo "ALTER TABLE $db CHANGE COLUMN VICIDIAL_park_on_filename OSDIAL_park_on_filename VARCHAR(10) default 'park';" | mysql -u root > /dev/null 2>&1
if [ "$?" -gt 0 ]; then echo -c "x"; else echo -n "."
echo "ALTER TABLE $db CHANGE COLUMN VICIDIAL_web_URL OSDIAL_web_URL VARCHAR(255) default 'http://localhost/test_VICIDIAL_output.php';" | mysql -u root > /dev/null 2>&1
if [ "$?" -gt 0 ]; then echo -c "x"; else echo -n "."
echo "DONE"
echo
