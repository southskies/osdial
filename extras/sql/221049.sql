...
ALTER TABLE system_settings ADD enable_lead_allocation ENUM('0','1') default '1';##|##
 ##    Option to turn the Lead Allocation menu on / off, default to on.;

ALTER TABLE system_settings ADD enable_external_agents ENUM('0','1') default '0';##|##
 ##    Option to turn the External Agents menu on / off, defaults to off.;

ALTER TABLE system_settings ADD enable_filters ENUM('0','1') default '1';##|##
 ##    Option to turn the SQL Filter menu on / off, default to on.;

ALTER TABLE system_settings ADD enable_multicompany ENUM('0','1') default '0';##|##
 ##    Option to turn multi-company support on / off, default to off.;
...
UPDATE system_settings SET version='2.2.1.049',last_update_check=DATE_SUB(NOW(), INTERVAL 1 DAY);##|##
 ##    Updating database to version 2.2.1.049 and clearing last_update_check flag.;
